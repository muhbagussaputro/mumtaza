    ...
    <div class="bg-gradient-to-r from-teal-500 to-teal-600 text-white">
        <div class="px-4 py-6">
            <div class="flex items-center">
                ...
                <div>
                    <h1 class="text-xl font-bold">Edit Guru</h1>
                    <p class="text-teal-100 text-sm">Edit data guru {{ $guru->nama }}</p>
                </div>
            </div>
        </div>
    </div>
                ...
                <form action="{{ route('admin.gurus.update', $guru) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex items-center space-x-4">
                            <div class="w-20 h-20 bg-gray-100 rounded-full flex items-center justify-center" id="preview-container">
                                @if($guru->foto_profil)
                                    <img src="{{ Storage::url($guru->foto_profil) }}" alt="{{ $guru->nama }}" class="w-20 h-20 rounded-full object-cover">
                                @else
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <input type="date" name="tanggal_lahir" id="tanggal_lahir" value="{{ old('tanggal_lahir', $guru->tanggal_lahir?->format('Y-m-d')) }}" 
                    ...
                    <div class="mb-6">
                        ...
                        <textarea name="alamat" id="alamat" rows="3" 
                                  class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 @error('alamat') border-red-500 @enderror">{{ old('alamat', $guru->alamat) }}</textarea>
                        @error('alamat')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                        ...
                        <div>
                            ...
                            <select name="status" id="status" required 
                                ...
                                <option value="active" {{ old('status', $guru->status) == 'active' ? 'selected' : '' }}>Aktif</option>
                                <option value="inactive" {{ old('status', $guru->status) == 'inactive' ? 'selected' : '' }}>Tidak Aktif</option>
                            </select>
                            @error('status')
                                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                            @enderror
